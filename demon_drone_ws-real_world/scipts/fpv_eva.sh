...
source ~/demon_packages/SGP_Planner_ws/devel/setup.bash

gnome-terminal -t "mavros" --window -e 'bash -c "roslaunch mavros px4.launch fcu_url:="/dev/ttyUSB0:921600" gcs_url:=udp-b://@; exec bash"' \
--tab -t "stereo"        -e 'bash -c "sleep 3; roslaunch stereo stereo.launch           ; exec bash"' \
--tab -t "vins"          -e 'bash -c "sleep 5; roslaunch vins stereo.launch             ; exec bash"' \
--tab -t "commu_bridge"  -e 'bash -c "sleep 7; roslaunch commu_bridge transfer.launch   ; exec bash"' \
--tab -t "Eva"           -e 'bash -c "sleep 9; roslaunch SGP_Planner planner.launch  ; exec bash"' \
...
# --tab -t "vins_rivz"     -e 'bash -c "sleep 5; source ~/Escort_Drone_ws/devel/setup.bash; roslaunch vins vins_rviz.launch               ;    exec bash"' \
...
# sleep 4
# rosrun mavros mavcmd long 511 31 3000 0 0 0 0 0
